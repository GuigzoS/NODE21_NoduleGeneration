...
def convert_image_np(inp, mini=-1, maxi=1):
    if inp.shape[1]==3:
        #inp = denorm(inp)
...
def read_image(opt):
    if ('%s/%s' % (opt.input_dir,opt.input_name)).endswith(".npy"):
        x = np.load('%s/%s' % (opt.input_dir,opt.input_name))
    elif ('%s/%s' % (opt.input_dir,opt.input_name)).endswith(".nii.gz"):
        x = nib.load('%s/%s' % (opt.input_dir,opt.input_name)).get_fdata()
    elif ('%s/%s' % (opt.input_dir,opt.input_name)).endswith(".mha"):
        x,_ = load('%s/%s' % (opt.input_dir,opt.input_name))
    else:
...
def read_image_dir(dir,opt):
    if dir.endswith(".npy"):
        x = np.load(dir)
    elif dir.endswith(".nii.gz"):
        x = nib.load(dir).get_fdata()
    elif dir.endswith(".mha"):
        x,_ = load(dir)
    else:
        x = img.imread(dir)
    x = np2torch(x,opt)
...
def np2torch(x,opt):
    if opt.nc_im == 3:
        ...
        x = x.transpose(3, 2, 0, 1)
        #if x.max()>50:
        #    #print("max>50 : "+str(x.max()))
        #    x = x/255
        if x.max()>1:
            x = (x-x.min())/(x.max()-x.min())*2-1
            #print("Potentiellement mauvaise normalisation (x.max()>1 mais <50 ; cf functions.np2torch) : "+str(x.max()))
    ...
    #print("---------im----------")
    #print(x)
    #print("FUNC------min "+str(x.min().item())+"-------max "+str(x.max().item())+"----------")
    return x
...
def save_as_nifti(arr, header, path2save):
    nib.save(nib.Nifti1Image(arr, None, header), path2save)

def save_as_mha(arr, header, path2save):
    save(arr, path2save, hdr=header)
...
def load_trained_pyramid(opt, mode_='train'):
    #dir = 'TrainedModels/%s/scale_factor=%f' % (opt.input_name[:opt.extension_size], opt.scale_factor_init)
    mode = opt.mode
    opt.mode = 'train'
...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = opt.input_dir#/%s/scale_factor=%f,alpha=%d' % (opt.input_name[:opt.extension_size], opt.scale_factor_init,opt.alpha)
    elif (opt.mode == 'animation_train') :
        dir2save = 'TrainedModels/%s/scale_factor=%f_noise_padding' % (opt.input_name[:opt.extension_size], opt.scale_factor_init)
    elif (opt.mode == 'paint_train') :
        dir2save = 'TrainedModels/%s/scale_factor=%f_paint/start_scale=%d' % (opt.input_name[:opt.extension_size], opt.scale_factor_init,opt.paint_start_scale)
    elif opt.mode == 'random_samples':
        dir2save = '%s/RandomSamples/%s/gen_start_scale=%d' % (opt.out,opt.input_name[:opt.extension_size], opt.gen_start_scale)
    elif opt.mode == 'random_samples_arbitrary_sizes':
        dir2save = '%s/RandomSamples_ArbitrerySizes/%s/scale_v=%f_scale_h=%f' % (opt.out,opt.input_name[:opt.extension_size], opt.scale_v, opt.scale_h)
    elif opt.mode == 'animation':
        dir2save = '%s/Animation/%s' % (opt.out, opt.input_name[:opt.extension_size])
    elif opt.mode == 'SR':
        dir2save = '%s/SR/%s' % (opt.out, opt.sr_factor)
    elif opt.mode == 'harmonization':
        if opt.harmonization_start_scale != -1:
            dir2save = '%s/%s/%s' % (opt.out, opt.experiment_name, "scale_"+str(opt.harmonization_start_scale))
        else:
            dir2save = '%s/%s' % (opt.out, opt.experiment_name)
    elif opt.mode == 'editing':
        dir2save = '%s/Editing/%s' % (opt.out, opt.input_name[:opt.extension_size])
    elif opt.mode == 'paint2image':
        dir2save = '%s/Paint2image/%s' % (opt.out, opt.input_name[:opt.extension_size])
...
def post_config(opt):
    ...
    opt.extension_size = -7 if opt.input_name.endswith(".nii.gz") else -4 #added
    opt.nc_z = opt.nc_im #added
    opt.scale_factor_init = opt.scale_factor
    opt.out_ = 'TrainedModels/%s/scale_factor=%f/' % (opt.input_name[:opt.extension_size], opt.scale_factor)
    if opt.mode == 'SR':
        opt.alpha = 100
...
def dilate_mask(mask,opt):
    ...
    if DEBUG:
        ...
        print(mask.max())
    opt.nc_im = nc_im
    mask = mask.expand(1, opt.nc_z, mask.shape[2], mask.shape[3])
    #plt.imsave('%s/%s_mask_dilated.png' % (opt.ref_dir, opt.ref_name[:opt.extension_size]), convert_image_np(mask), vmin=0,vmax=1)
